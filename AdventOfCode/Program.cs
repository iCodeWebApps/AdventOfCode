...
namespace AdventOfCode
{
    public class Program
    {
        static void Main(string[] args)
        {
            ...
            var _grid = new int[20000, 20000];

            var origin = _grid.GetOrigin();

            _grid
                .AddRoute(origin, x.AsRoute())
                .AddRoute(origin, y.AsRoute());

            var intersections = _grid.GetIntersections();

            var totalDistanceAways = new List<int>();

            foreach (var intersection in intersections)
            {
                var distanceAwayRow = System.Math.Abs(intersection.Row - origin.Row);
                var distanceAwayColumn = System.Math.Abs(intersection.Column - origin.Column);
    ...
    {
        public static List<Coordinate> GetIntersections(this int[,] grid)
        {
